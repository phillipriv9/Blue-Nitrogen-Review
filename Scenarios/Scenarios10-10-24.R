...
# N densities based on Model1 of Table 3 considering the top 1 m. 
Ndensitymarsh=1.946
Ndensitymangrove=1.939

# Calculate nitrogen gain for marshes and mangroves in Tg
historical$Ngainmarsh <- ((historical$Amarsh) * (historical$SL / 10) * (Ndensitymarsh / 1000) * 10^10) / 10^12
historical$Ngainmangrove <- ((historical$Amangrove) * (historical$SL / 10) * (Ndensitymangrove / 1000) * 10^10) / 10^12
...
historical$Nlossmarsh <- ifelse(   
  ...
  (historical$Amarsh - historical$A2020marsh) * 30 * (Ndensitymarsh / 1000) * 10^10 / 10^12,
  0
)

historical$Nlossmangrove <- ifelse(
  historical$Amangrove > historical$A2020mangrove,
  (historical$Amangrove - historical$A2020mangrove) * 30 * (Ndensitymangrove / 1000) * 10^10 / 10^12, 
  0
)
...
scenarios$Ngainmarsh <- ((scenarios$Amarsh) * (scenarios$SL/10) * (Ndensitymarsh/1000) *10^10)/10^12
scenarios$Ngainmangrove <- ((scenarios$Amangrove) * (scenarios$SL/10) * (Ndensitymangrove/1000) * 10^10)/10^12
scenarios$Ngaintotal <- scenarios$Ngainmarsh + scenarios$Ngainmangrove


scenarios$Ngaintotal <- scenarios$Ngainmarsh + scenarios$Ngainmangrove

# Calculate N loss using Ndensity for marsh, mangrove, assuming 50 cm of erosive loss from area that is lost
scenarios$Nlossmarsh <- ifelse(   
  scenarios$Amarsh < scenarios$A2020marsh,
  (scenarios$Amarsh - scenarios$A2020marsh) * 50 * (Ndensitymarsh / 1000) * 10^10 / 10^12,
  0)
scenarios$Nlossmangrove <- ifelse(
  scenarios$Amangrove < scenarios$A2020mangrove,
  (scenarios$Amangrove - scenarios$A2020mangrove) * 50 * (Ndensitymarsh / 1000) * 10^10 / 10^12,
  0
)
...
# Assuming 'combined_data' contains your data
# Filter the data for year 2100 and get the netNtotal values for each scenario
values_at_2100 <- combined_data %>%
  filter(year == 2100) %>%
  select(scenarioname, netNtotal)

# View the results
print(values_at_2100)
...
contour_grid$Ngainmarsh <- ((contour_grid$Amarsh) * (contour_grid$SL/10) * (Ndensitymarsh/1000) * 10^10) / 10^12
contour_grid$Ngainmangrove <- ((contour_grid$Amangrove) * (contour_grid$SL/10) * (Ndensitymangrove/1000) * 10^10) / 10^12
...
contour_grid$Nlossmarsh <- ifelse(   
  ...
  (contour_grid$Amarsh - 52880) * 50 * (Ndensitymarsh / 1000) * 10^10 / 10^12,0)
contour_grid$Nlossmangrove <- ifelse(
  contour_grid$Amangrove < 147349,
  (contour_grid$Amangrove - 147349) * 50 * (Ndensitymangrove / 1000) * 10^10 / 10^12,0)
...
# Save the net N plots
ggsave("outputs/3.projections.pdf", plot = fig3, width = 10, height = 5)
