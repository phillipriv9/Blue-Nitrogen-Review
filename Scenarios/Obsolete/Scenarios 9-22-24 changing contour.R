...
Atotal_values <- seq(min(5000), max(350000), by = 5000)  # Amangrove increments of 5000 for better resolution

contour_grid <- expand.grid(
  SL = SL_values,
  Atotal = Atotal_values
  )

# Calculate N gain for the contour 
contour_grid$Ngaintotal <- ((contour_grid$Atotal) * (contour_grid$SL/10) * (1.7/1000) * 10^10) / 10^12

# Calculate N loss for the contour 
contour_grid$Nlosstotal <- ifelse(   
  contour_grid$Atotal < 200239,
  (contour_grid$Atotal - 200239) * 50 * (1.78 / 1000) * 10^10 / 10^12,0)

# Calculate net N change for the contour 
contour_grid$netNtotal <- contour_grid$Ngaintotal + contour_grid$Nlosstotal
...
plot_elev_area <- ggplot() +
  ...
  scale_fill_gradientn(
    colors = c("orange", "white", "darkgreen"), # More colors can be added here
  ...
  # Customize labels
  labs(title = NULL, y = expression("Wetland elevation change (mm)")) +
