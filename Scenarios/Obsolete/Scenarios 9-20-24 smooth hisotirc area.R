#this version has a sophisticated but maybe accurate smooth historic wetland area curve that doesn't yet work with the other code. 
...
scenarios <- data.frame(
  ...
  scenarioname = rep(c('Area loss/low slr', 'Area loss/high slr', 'Area stable/low slr', 
                     ...
                     each = length(years)),
  R2020 = 3.5,
  R2100 = rep(c(4.1, 8, 4.1, 8, 4.1, 8), each = length(years)),
  A2020marsh = 52880,
...
# Calculate Ndensity and CN
df1 <- df1 %>% 
  group_by(Habitat) %>%
  mutate(Ndensity = (N_perc / 100) * BD_reported_g_cm3)

# Calculate averages for marsh and mangrove habitats
marshNdensity <- df1 %>% 
  filter(Habitat == "marsh") %>%
  summarise(average_Ndensity = mean(Ndensity, na.rm = TRUE))

mangroveNdensity <- df1 %>% 
  filter(Habitat == "mangrove") %>%
  summarise(average_Ndensity = mean(Ndensity, na.rm = TRUE))

marshCN <- df1 %>% 
  filter(Habitat == "marsh") %>%
  summarise(average_CN = mean(CN, na.rm = TRUE))
...
# Extract the average density values as numeric
marshNdensity_ave <- marshNdensity$average_Ndensity
mangroveNdensity_ave <- mangroveNdensity$average_Ndensity
...
historical_area <- data.frame(historical_year = seq(1700, 2019, by = 5))  # Replace with your actual data

# Initialize columns for Amarsh and Amangrove
historical_area$Amarsh <- NA
historical_area$Amangrove <- NA

# Calculate Amarsh and Amangrove for each year

  historical_area$Amarsh <- with(wetland_area_model[1, ], 
                               Lowerasympt[1] + (Upperasympt[1] - Lowerasympt[1]) * 
                                 exp(-exp(-Growthrate[1] * (historical_area$historical_year - Inflection[1])))
  )  
  
  historical_area$Amangrove <- with(wetland_area_model[2, ], 
                                    Lowerasympt[1] + (Upperasympt[1] - Lowerasympt[1]) * 
                                      exp(-exp(-Growthrate[1] * (historical_area$historical_year - Inflection[1])))
  )  
  historical_area$Atotal_historical <- historical_area$Amarsh + historical_area$Amangrove


# View the results
print(historical_area)
...
historical$Ngainmarsh <- ((historical$Amarsh) * (historical$SL / 10) * (marshNdensity_ave / 1000) * 10^10) / 10^12
historical$Ngainmangrove <- ((historical$Amangrove) * (historical$SL / 10) * (mangroveNdensity_ave / 1000) * 10^10) / 10^12
...
historical$Nlossmarsh <- ifelse(   
  ...
  (historical$Amarsh - historical$A2020marsh) * 30 * (marshNdensity_ave / 1000) * 10^10 / 10^12,
  0
)

historical$Nlossmangrove <- ifelse(
  historical$Amangrove > historical$A2020mangrove,
  (historical$Amangrove - historical$A2020mangrove) * 30 * (mangroveNdensity_ave / 1000) * 10^10 / 10^12, 
  0
)
...
scenarios$Ngainmarsh <- ((scenarios$Amarsh) * (scenarios$rSLR/10) * (1.78/1000) *10^10)/10^12
scenarios$Ngainmangrove <- ((scenarios$Amangrove) * (scenarios$rSLR/10) * (1.63/1000) * 10^10)/10^12
...
scenarios$Ngaintotal <- scenarios$marshNgain + scenarios$mangroveNgain
...
scenarios$netNmarsh <- scenarios$marshNgain + scenarios$Nlossmarsh
scenarios$netNmangrove <- scenarios$mangroveNgain + scenarios$Nlossmangrove
...
rSLR_values <- seq(0, 9, by = .05)  # SL increments of 0.1 for smoothness
