...
rSLR_values <- seq(3.5, max(scenarios$rSLR), by = .05)  # SL increments of 0.1 for smoothness
Amarsh_values <- seq(min(30000), max(100000), by = 5000)  # Amarsh increments of 500 for better resolution
Amangrove_values <- seq(min(80000), max(300000), by = 5000)  # Amangrove increments of 500 for better resolution
...
contour_grid$SL <- contour_grid$rSLR*38.8 + 256
...
plot_elev_area <- ggplot() +
  ...
  geom_line(data = scenarios, aes(x = Atotal, y = rSLR, linetype = scenarioname, group = scenario), color = "black") +  # Overlay the line plot
  #scale_color_manual(values = plot_colors) +

  
  # Black lines with varying dash patterns for different scenarios
  geom_line(data = scenarios, aes(x = Atotal, y = rSLR, linetype = scenarioname, group = scenario), color = "black") +  
  scale_linetype_manual(values = c("solid", "solid", "dotted", "dotted", "dashed", "dashed")) +  # Define different dash types
