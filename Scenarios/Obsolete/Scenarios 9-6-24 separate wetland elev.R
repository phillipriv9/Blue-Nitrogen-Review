...
scenarios <- scenarios %>%
  mutate(marshgain = ifelse(rSLR > 5, 0, rSLR),
         mangrovegain = ifelse(rSLR > 7.8, 0, rSLR))
...
plot_combined_elev_year <- ggplot(scenarios) +
  ...
  geom_line(aes(x = year, y = mangroveelev + 2, 
                color = "Mangrove", 
                linetype = as.factor(R2100), 
                group = interaction(R2100, "Mangrove"))) +
  
  # Line for marsh elevation at each R2100 level
  geom_line(aes(x = year, y = marshelev - 2, 
                color = "Marsh", 
                linetype = as.factor(R2100), 
                group = interaction(R2100, "Marsh"))) +
  
  # Define colors for mangrove vs marsh
  scale_color_manual(values = c("Mangrove" = "salmon", "Marsh" = "lightseagreen")) +
  ...
  # Customize legends for both color (mangrove vs marsh) and linetype (R2100 levels)
  guides(
    color = guide_legend(title = "Elevation Type"),
    linetype = guide_legend(title = "R2100 Levels")
  )
...
plot_elev_area <- ggplot() +
  ...
  scale_fill_viridis_c(-1) +  # Apply continuous color scale for fill
  ...
  scale_linetype_manual(values = c("dashed", "dashed", "dotted", "dotted", "solid", "solid")) +  
  
  labs(title = NULL, x = "Total Area", y = "Wetland elevation change (mm yr-1") +
  ...
  theme(
    ...
    axis.ticks.length = unit(0.15, 'cm'),  # Set shorter tick length
