...
#I want this to generate a smooth countour for the background of plot_elev_area, so i need to plot thos lines over top of this countour.
# Create a grid of values
SL_values <- seq(3.5, max(scenarios$rSLR), by = 1)  # SL increments of 1
Amarsh_values <- seq(min(50000), max(150000), by = 1000)  # Amarsh increments of 1000
Amangrove_values <- seq(min(150000), max(300000), by = 1000)  # Amangrove increments of 1000
...
# Plot with the contour background
plot_elev_area <- ggplot() +
  geom_tile(data = contour_grid, aes(x = Atotal, y = SL, fill = netNtotal), width = 100, height = 0.1) +  # Background tiles
  scale_fill_viridis_c(option = "D", direction = -1) +  # Apply a color scale for fill
